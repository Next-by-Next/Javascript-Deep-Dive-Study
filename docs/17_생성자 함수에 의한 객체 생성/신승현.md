...
이번에는 객체 생성 방식 중에서 생성자 함수를 사용하여 객체를 생성하는 방식을 살펴보고 객체 리터럴을 사용하여 생성하는 방식과 생성자 함수를 사용하여 객체를 생성하는 방식과의 장단점을 살펴보자.
```
...
- 객체 생성하는 방법은 **객체 리터럴을 사용하는 것이 더 간편하다.**
...
- new 연산자와 함께 생성자 함수를 호출하면 자바스크립트 엔진은 암묵적으로 인스턴스를 생성하고 인스턴스를 초기화한 후 암묵적으로 인스턴스를 반환한다.

> ##### 2.3.1 인스턴스 생성과 this 바인딩

- 암묵적으로 빈 객체가 생성된다.
- 이 빈 객체가 바로(아직 생성 안됨) 생성자 함수가 생성한 인스턴스다.
...
- this에 바인딩되어 있는 인스턴스에 프로퍼티나 메서드를 추가하고 생성자 함수가 인수로 전달받은 초기값을 인스턴스 프로퍼티에 할당하여 고정값을 할당한다.
...
- 물론 함수로서 동작하기 위해 함수 객체만을 위한 [[Environment]], [[FormalParameters]] 등의 내부 슬롯과 [[Call]], [[Constructor]] 같은 내부 메서드를 추가로 가지고 있다.
...
- 함수 선언문과 함수 표현식으로 정의된 함수만이 constructor이고 ES6의 화살표 함수와 메서드 축약 표현으로 정의된 함수는 non-consturctor다.
...
- 주의할 것은 생성자 함수로서 호출될 것을 기대하고 정의하지 않은 일반함수에 new 연산자를 붙여 호출하면 생성자 함수처럼 동작할 수 있다는 것이다.
...
- 생성자 함수가 new 연산자 없이 호출되는 것을 방지하기 위해 파스칼 케이스 컨벤션을 사용한다 하더라도 실수는 발생하기에 ES6에서 `new.target`을 지원한다.
...
- new 연산자와 함께 생성자 함수에 의해 생성된 객체(인스턴스)는 프로토타입에 의해 생성자 함수와 연결되어 new 연산자와 함께 호출되었는지 확인할 수 있다.

---

> ### 느낀 점

저번 챕터와 마찬가지로 자바스크립트 내부 엔진에 관련된 부분에 있어서 한 번 읽어서 완전히 이해하진 못하였다. 반복 숙달하여 익힐 때까지 부지런히 공부해야겠다. 큰 맥락에 있어서 객체 생성의 방법과 방법간의 장단점을 배울 수 있었다.
