...
객체에는 다양한 생성 방법이 있다. 세부적인 객체 생성 방식의 차이는 있으나 모두 추상 연산 OrdinaryObjectCreate에 의해 생성된다는 공통점이 있다. 해당 추상 연산에는 **생성할 객체의 프로토타입을 필수로 전달해야 하는데, 이 인수는 객체가 생성되는 시점에 객체 생성 방식에 의해 결정된다**.

<br/>

### 1. 객체 리터럴에 의해 생성된 객체의 프로토타입

위에서 OrdinaryObjectCreate에서 전달되는 프로토타입은 객체 생성 시점에 생성 방식에 의해 결정된다고 했다. 따라서 객체 리터럴에 의해 생성된 객체의 프로토타입은 Object.prototype이 된다.

**OrdinaryObjectCreate이 실행된 결과는 Object 생성자 함수와 Object.prototype, 그리고 생성된 객체 사이의 연결이다**.

### 2. Object 생성자 함수에 의해 생성된 객체의 프로토타입

Object 생성자 함수를 호출하면 객체 리터럴과 마찬가지로 OrdinaryObjectCreate가 호출되고 Object 생성자 함수와 Object.prototype, 생성된 객체 사이에 연결이 이루어진다.

객체 리터럴과 Object 생성자 함수에 의한 객체 생성 방식의 차이는 프로퍼티를 추가하는 방식에 있다. **객체 리터럴은 내부에 프로퍼티를 추가하지만 Object 생성자는 일단 빈 객체를 생성한 이후 프로퍼티를 추가해야 한다**.
...
new 연산자와 함께 생성자 함수를 호출하여 인스턴스를 생성하면, **OrdinaryObectCreate의 인자로 생성자 함수의 prototype 프로퍼티에 바인딩되어 있는 객체가 전달된다**. 이때 생성자 함수의 prototype 프로퍼티의 값과 Object.prototype는 차이가 있다. Object.prototype은 다양한 빌트인 메서드를 갖고 있지만 **생성자 함수의 prototype은 constructor 뿐이다**.
